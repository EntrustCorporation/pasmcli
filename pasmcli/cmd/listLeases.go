...
// listLeasesCmd represents the list-lease command
var listLeasesCmd = &cobra.Command{
    Use:   "list-leases",
    Short: "List all lease details",
    Run: func(cmd *cobra.Command, args []string) {
        ...
        // now POST
        endpoint := GetEndPoint("", "1.0", "ListLeases")
...
func init() {
    ...
    listLeasesCmd.Flags().StringP("filters", "l", "",
                                 "Conditional expression to list filtered " +
                                 "leases")
    listLeasesCmd.Flags().IntP("max-items", "m", 0,
                              "Maximum number of items to include " +
                              "in the response")
    listLeasesCmd.Flags().StringArrayP("field", "f", []string{},
                                      "Lease field to include in the response")
    listLeasesCmd.Flags().StringP("next-token", "n", "",
