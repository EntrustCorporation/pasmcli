...
// listBoxIdsCmd represents the list-box-ids command
var listBoxIdsCmd = &cobra.Command{
    Use:   "list-box-ids",
    Short: "List all box(box id & name only) details",
    Run: func(cmd *cobra.Command, args []string) {
        ...
        // now POST
        endpoint := GetEndPoint("", "1.0", "ListBoxIds")
...
func init() {
    rootCmd.AddCommand(listBoxIdsCmd)
    listBoxIdsCmd.Flags().StringP("prefix", "p", "",
                                  "List only those Boxes whose name starts " +
                                  "with this string")
    listBoxIdsCmd.Flags().StringP("filters", "f", "",
                                  "Conditional expression to list filtered " +
                                  "Boxes")
    listBoxIdsCmd.Flags().IntP("max-items", "m", 0,
                               "Maximum number of items to include in " +
                               "response")
    listBoxIdsCmd.Flags().StringP("next-token", "n", "",
