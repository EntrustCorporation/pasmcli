...
// createPwdSecretCmd represents the create-pwd-secret command
// static secret
var createPwdSecretCmd = &cobra.Command{
    Use:   "create-pwd-secret",
    Short: "Create a Vault Password Secret within the specified box",
    Run: func(cmd *cobra.Command, args []string) {
        ...
        // get pwd string
        pwd, _ := flags.GetString("password")
        params["secret_data"] = pwd
...
func init() {
    rootCmd.AddCommand(createPwdSecretCmd)
    createPwdSecretCmd.Flags().StringP("boxid", "b", "",
                                    "Id or name of the Box")
    createPwdSecretCmd.Flags().StringP("name", "n", "",
                                    "Name of the Secret")
    createPwdSecretCmd.Flags().StringP("description", "d", "",
                                    "Short description for the Secret")
    createPwdSecretCmd.Flags().StringP("password", "p", "",
                                    "Secret password data")

    createPwdSecretCmd.Flags().StringArrayP("tagkey", "t", []string{},
                                         "Tag key to associate with the Secret. " +
                                         "This option is repeatable.")
    createPwdSecretCmd.Flags().StringArrayP("tagvalue", "v", []string{},
                                         "Tag value to associate with the Secret. " +
                                         "This option is repeatable.")
    createPwdSecretCmd.Flags().StringP("expires_at", "e", "",
                                    "Expiration time in RFC 3339 format.")

    // mark mandatory fields as required
    createPwdSecretCmd.MarkFlagRequired("boxid")
    createPwdSecretCmd.MarkFlagRequired("name")
    createPwdSecretCmd.MarkFlagRequired("password")
}