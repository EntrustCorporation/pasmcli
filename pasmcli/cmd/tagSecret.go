...
// tagSecretCmd represents the tag-secret command
var tagSecretCmd = &cobra.Command{
    Use:   "tag-secret",
    Short: "Add/update tags to Secret",
    Run: func(cmd *cobra.Command, args []string) {
        ...
        // now POST
        endpoint := GetEndPoint("", "1.0", "TagSecret")
...
func init() {
    rootCmd.AddCommand(tagSecretCmd)
    tagSecretCmd.Flags().StringP("boxid", "b", "",
                                "Id or name of the Box under which the " +
                                "Secret is")
    tagSecretCmd.Flags().StringP("secretid", "s", "",
                               "Id or name of the Secret to tag")
    tagSecretCmd.Flags().IntP("revision", "R", 0,
                           "Revision number of the Secret")
    tagSecretCmd.Flags().StringArrayP("tagkey", "t", []string{},
                                   "Tag key to associate with the Secret. " +
                                   "This option is repeatable.")
    tagSecretCmd.Flags().StringArrayP("tagvalue", "v", []string{},
                                   "Tag value to associate with the Secret. " +
                                   "This option is repeatable.")

    // mark mandatory fields as required
    tagSecretCmd.MarkFlagRequired("boxid")
    tagSecretCmd.MarkFlagRequired("secretid")
    tagSecretCmd.MarkFlagRequired("revision")
    tagSecretCmd.MarkFlagRequired("tagkey")
    tagSecretCmd.MarkFlagRequired("tagvalue")
}
